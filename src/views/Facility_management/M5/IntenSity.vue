<template>
  ...
  <div>
    ...
    <h3 v-if="IntenseData.length > 0">最小值: <span style="color: goldenrod">{{ getMinValue() }}</span></h3>
...
export default {
  ...
  methods: {
    ...
    getData() {
      ...
      const url = `http://10.112.6.250:8888/api/v1/sensor?dev_id=1000004700030003&begin_time=${beginTime}&end_time=${endTime}`;
    ...
    initChart() {
      this.chart = echarts.init(document.getElementById('intensity'));
      this.drawChart();
    },
    getMaxValue() {
